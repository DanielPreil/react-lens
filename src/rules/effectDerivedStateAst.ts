...
function getFunctionBodyRange(fn: TSESTree.ArrowFunctionExpression | TSESTree.FunctionExpression): [number, number] | null {
...
function buildDerivedStateFinding(
  ...
  return buildFinding(source, index, {
    ...
    pattern: "A useState setter is called inside useEffect to mirror render-available data.",
    whyItMatters: "This usually adds an extra render pass and can create stale state bugs.",
    suggestion: "Compute the value during render. Keep useMemo only for genuinely expensive calculations.",
    whenToIgnore: "Ignore when the Effect is intentionally syncing with an external system, not deriving UI state.",
...
export const effectDerivedStateAstRule: Rule = {
  ...
  run: ({ source, rawSource }) => {
    ...
    traverse(ast, (node) => {
      ...
      const hasAsyncSignals = callback.async || /\bawait\b|\.then\s*\(/.test(effectText);
      ...
      const hasDependencies = deps?.type === "ArrayExpression" && deps.elements.length > 0;
      ...
      const confidence: "high" | "medium" = hasDependencies && setterCalls.length === 1 ? "high" : "medium";
      findings.push(buildDerivedStateFinding(original, setterCalls[0], confidence));
    });
